(function ($) {
    var $View = function (str) {
        ...
        if (el.attr('id')) {
            ...
            return prefix + '_ui_name_' + el.attr('name').replace('[]', '').replace('[', '_').replace('\']', '').replace(']', '').replace('\'', '').replace('"', '');
        } else {
        ...
        return $(this).each(function () {
            var $thisEl = $(this), $optsEl = $thisEl.children('option'), $uiEl, thisEvs = $thisEl.on('events'),
                uiId = createUiId($thisEl, 'select'), optsWidth = 0, optsPosition, optionsPosition, optionsLimit,
                tpl = {
                ...
                }, $selectEl = $View(tpl.select), $sLtEl = $View(tpl.selectLt), $sRtEl = $View(tpl.selectRt),
                $optionsEl = $View(tpl.options), $optionsInnerEl = $View(tpl.optionsInner), init = function () {
                    if ($thisEl.length <= 1 && $thisEl.get(0).tagName === 'SELECT' && !$thisEl.attr('multiple') && $thisEl.children('optgroup').length <= 0) {
                        ...
                        $optsEl.each(function (i) {
                            var optEl = $(this), optionTxt = optEl.text(),
                        ...
                        $thisEl.setOptions = function () {
                            ...
                            var top, sltPosition = $selectEl.position(),
                                ...
                                eachOptHeight = $optionsInnerEl.outerHeight() / $optsEl.length, optsHeight = 'auto';
                            if (optionsLimit) {
                            ...
                            if (optsPosition && ($(window).height() + $(document).scrollTop() < $optionsEl.offset().top + $optionsEl.outerHeight())) {
